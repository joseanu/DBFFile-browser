...
describe('Reading a DBF file', () => {
    ...
    async function getMemoArrayBuffer(dbfFilePath: string): Promise<ArrayBuffer | undefined> {
        const { dir, name } = path.parse(dbfFilePath);
        const fptPath = path.join(dir, name + '.fpt');
        const dbtPath = path.join(dir, name + '.dbt');
        try { return toArrayBuffer(await fs.readFile(fptPath)); } catch { /* File not found */ }
        try { return toArrayBuffer(await fs.readFile(dbtPath)); } catch { /* File not found */ }
        return undefined;
    }

    tests.forEach(test => {
        it(`readRecords: ${test.description}`, async () => {
            const { filename, options, recordCount, dateOfLastUpdate, numberOfRecordsToRead, firstRecord, lastRecord, deletedCount, error } = test;
            ...
            try {
                ...
                const dbf = await DBFFile.open(dbfArrayBuffer, memoArrayBuffer, options);
                ...
                if (lastRecord && DELETED in lastRecord) {
                    expect(records[records.length - 1][DELETED]).toBe(lastRecord[DELETED]);
                }

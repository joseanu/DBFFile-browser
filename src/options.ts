...
export function normaliseOpenOptions(
  ...
  if (typeof includeDeletedRecords !== "boolean") {
    throw new Error(
      `Invalid value 'includeDeletedRecords' value ${includeDeletedRecords}`,
    );
  }
...
export function normaliseCreateOptions(
  ...
  if (typeof encoding === "string") {
    ...
    for (let key of Object.keys(encodingObject)) {
      ...
      try {
        ...
        throw new Error(
          `Unsupported character encoding '${encodingObject[key]}' for field '${key}'`,
        );
      }
    }
  } else {
